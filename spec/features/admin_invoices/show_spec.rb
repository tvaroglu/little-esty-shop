...
RSpec.describe 'Admin Invoice Show Page' do
  ...
  it 'is on the correct page' do
    expect(page).to have_current_path("/admin/invoices/#{@invoice1.id}", ignore_query: true)
  ...
  it 'can update invoice status (happy path)' do
    ...
    expect(page).to have_current_path(admin_invoice_path(@invoice1.id), ignore_query: true)
