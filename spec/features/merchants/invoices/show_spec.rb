...
RSpec.describe 'The Merchant Invoice show page' do
  ...
  it 'is on the correct page' do
    expect(page).to have_current_path(merchant_invoice_path(@merchant1.id, @invoice1.id), ignore_query: true)
  end
  ...
  it "displays a link to return to the invoices' index page" do
    ...
    expect(page).to have_current_path(merchant_invoices_path(@merchant1.id), ignore_query: true)
  end

  it 'displays the invoice attributes' do
    expect(page).to have_content("Invoice ID: #{@invoice1.id}")
    expect(page).to have_content(@invoice1.status.capitalize.to_s)
    expect(page).to have_content(@invoice1.format_date(@invoice1.created_at).to_s)
    expect(page).to have_content(@invoice1.customer.first_name.to_s)
    expect(page).to have_content(@invoice1.customer.last_name.to_s)
  end
  ...
  describe 'links to discount show pages' do
    ...
    it 'can take the user to the applied discount for each invoite item' do
      within("div#id-#{@invoice_item5.id}") do
        ...
        expect(page).to have_current_path(merchant_discount_path(@merchant1, @discount1), ignore_query: true)
      end
    end
  end
end
