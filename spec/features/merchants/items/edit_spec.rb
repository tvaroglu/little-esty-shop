...
RSpec.describe 'The Merchant Item edit page' do
  ...
  describe 'Merchant Item Update' do
    ...
    it 'displays a form filled in with the existing item attribute information' do
      ...
      expect(page).not_to have_field('Name', with: @item2.name)
      expect(page).not_to have_field('Description', with: @item2.description)
      expect(page).not_to have_field('Unit price', with: @item2.unit_price)
    end
    ...
    it 'updates submitted are displayed on the item show page along with a flash message confirming the update' do
      ...
      expect(page).not_to have_content('Item successfully updated.')
      ...
      expect(page).to have_current_path(merchant_item_path(@item1.merchant_id, @item1.id), ignore_query: true)
      expect(page).not_to have_content('SK2')
